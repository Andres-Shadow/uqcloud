...
// 	err := GormDB.Model(&models.Maquina_virtual{}).Select("m.nombre, m.fecha_creacion, m.host_id, m.persona_email").
...
// func GetHost(idHost int) (models.Host, error) {
// 	var host models.Host
// 	err := GormDB.Where("id = ?", idHost).First(&host).Error
// 	if err != nil {
...
// func GetVM(nameVM string) (models.Maquina_virtual, error) {
// 	var maquinaVirtual models.Maquina_virtual
// 	err := GormDB.Where("nombre = ?", nameVM).First(&maquinaVirtual).Error
// 	if err != nil {
// 		if err == gorm.ErrRecordNotFound {
// 			log.Println("No se encontró la máquina virtual con el nombre especificado.")
// 		} else {
...
// 	if persona.Rol == "Administrador" {
// 		err = GormDB.Model(&models.Maquina_virtual{}).Select("m.nombre, m.ram, m.cpu, m.ip, m.estado, d.sistema_operativo, d.distribucion_sistema_operativo, m.hostname").
